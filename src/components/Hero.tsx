...
const Hero: React.FC = () => {
  ...
  const shouldOpenLogin = useMemo(() => {
    const params = new URLSearchParams(location.search);
    return params.get("admin") === "login";
  }, [location.search]);

  const [showLogin, setShowLogin] = useState(shouldOpenLogin);

  useEffect(() => {
    setShowLogin(shouldOpenLogin);
  }, [shouldOpenLogin]);
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10 py-8">
        <div
          className={`grid items-start gap-10 ${showLogin ? "lg:grid-cols-[1.2fr_0.8fr]" : "lg:grid-cols-1"}`}
        >
          <div className="max-w-2xl">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-slate-700 mb-8 leading-relaxed">
                ፍሪጅ፣ የልብስ ማጠቢያ ማሽን እና ሌሎች የቤት ውስጥ ኤሌክትሮኒክስ ዕቃዎችን በቤቶ መጥተን በጥራት
                እና በታማኝነት እንጠግናለን።
              </p>
              ...
              <div className="space-y-3">
                <div className="flex justify-end">
                  <Link
                    ...
                    className="rounded-full border border-slate-200 bg-white/90 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-blue-200 hover:text-blue-600"
                  >
                    Close login
                  </Link>
                </div>
                <Login compact redirectIfAuthenticated={false} />
              </div>
            </motion.div>
          )}
        </div>
